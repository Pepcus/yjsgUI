...
import {
  ...
  SINGLE_COT_ERROR_MESSAGE,
  DOUBLE_COT_ERROR_MESSAGE,
...
export const addressValidate = (value, name) => {
  ...
  if (isEmpty(value)) {
    ...
    errorMessageObject.message = SINGLE_COT_ERROR_MESSAGE;
    errorMessageObject[`isValid_${name}`] = false;
  } else if (value.includes('"')) {
    errorMessageObject.message = DOUBLE_COT_ERROR_MESSAGE;
    errorMessageObject[`isValid_${name}`] = false;
